...
<style>
    ...
    .header .badge { background: #ee4d2d; color: white; padding: 6px 16px; border-radius: 30px; font-size: 13px; font-weight: 600; }
    .product-card {
        background: white;
        border-radius: 16px;
        padding: 15px;
    ...
    .product-card .gambar { font-size: 50px; margin-bottom: 8px; }
    .product-card h3 { font-size: 16px; margin: 5px 0; color: #1a1a1a; }
    .product-card .harga { font-size: 20px; font-weight: 700; color: #ee4d2d; margin-bottom: 10px; }
    .stButton > button {
    ...
    .stButton > button:hover { background: #d43b1f !important; }
    .footer { text-align: center; padding: 20px 0 10px; color: #999; font-size: 13px; margin-top: 30px; border-top: 1px solid #eee; }
</style>
...
st.markdown("---")
st.markdown(f"## 🛒 Keranjang Belanja ({total_item} item)")
...
if len(st.session_state.keranjang) == 0:
    ...
    for idx, item in enumerate(st.session_state.keranjang):
        ...
        with col3:
            if st.button("✕", key=f"del_main_{idx}"):
    ...
    with st.form("form_pesan_main"):
        ...
        kelas = st.text_input("Kelas", value=st.session_state.kelas, key="main_kelas")
        no_hp = st.text_input("No. HP (WA)", value=st.session_state.no_hp, key="main_nohp")
